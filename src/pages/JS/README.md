...
| ---------------- | ------------------------------------------ | -------- | ----------------------------------------------- |
| **Array**        | `Array.some(callback)`                     | ❌ No    | Stops early, returns boolean                    |
| **Array**        | `Array.includes(value)`                    | ❌ No    | Returns `true / false`                          |
| **Array**        | `Array.filter(callback)`                   | ❌ No    | Returns a **new array** (`[a]` or `[]`)         |
| **Array**        | `Array.find(callback)`                     | ❌ No    | Stops early, returns **element or `undefined`** |
| **Array**        | `Array.reduce((acc, data) => {}, 0)`       | ❌ No\*  | Reducer may mutate accumulator                  |
| **Array**        | `Array.join()`                             | ❌ No    | Array → string                                  |
| **Array**        | `Array.pop()`                              | ✅ Yes   | Removes last element                            |
| **Array**        | `Array.shift()`                            | ✅ Yes   | Removes first element                           |
| **Array**        | `Array.sort()`                             | ✅ Yes   | Sorts in place                                  |
| **Array**        | `Array.splice()`                           | ✅ Yes   | Adds/removes items                              |
| **Array**        | `Array.forEach()`                          | ❌ No    | No early termination                            |
| **Array**        | `Array.map()`                              | ❌ No    | No early termination                            |
| **Control Flow** | `switch()`                                 | ❌ No    | Control statement                               |
| **Control Flow** | `switch(true)`                             | ❌ No    | Conditional switch                              |
| **Loop**         | `for (let i = 0; i < text.length; i++) {}` | ❌ No    | Classic index-based loop                        |
| **Object**       | `for...in`                                 | ❌ No    | Iterates over object keys                       |
| **String**       | `String.startsWith()`                      | ❌ No    | `str.startsWith(":") → true/false`              |
| **String**       | `String.endsWith()`                        | ❌ No    | `str.endsWith(":") → true/false`                |
| **String**       | `String.replace()`                         | ❌ No    | `str.replace(/\\+s/i, '')`                      |
| **String**       | `String.slice()`                           | ❌ No    | `":cry:".slice(1, -1) → "cry"`                  |
| **String**       | `String.split("")`                         | ❌ No    | Returns array                                   |
| **String**       | String char access                         | ❌ No    | Strings are **array-like**, read-only           |
| **String**       | `text[index]`                              | ❌ No    | `"tree"[0] → "t"`                               |
...
| Category   | Topic / Method          | Notes                                      |
| ---------- | ----------------------- | ------------------------------------------ |
| **Array**  | `Array.indexOf(value)`  | Returns index `n` or `-1`                  |
| **Date**   | `Date()` constructor    | `Date(y, m, d, h, min, s, ms)`             |
| **Date**   | Tomorrow                | `new Date(y, m, d + 1)`                    |
| **Date**   | `toLocaleDateString()`  | Example: `12/19/2025`                      |
| **Date**   | Last month (start)      | `new Date(y, m - 1, 1)`                    |
| **Date**   | Last month (end)        | `new Date(y, m, 0)`                        |
...
| **Math**   | Random index            | `Math.floor(Math.random() * array.length)` |
...
| **Math**   | `Math.random()`         | `0–99 → Math.floor(Math.random() * 100)`   |
...
| **Object** | Object merge (spread)   | `const output = { ...o1, ...o2 }`          |
...
| **Math**         | Remainder (`%`)                            |          | `const remainder = 10 % 3`                                  |
| **Math**         | `Math.round(2.4)`                          |          | `→ 2`                                                       |
| **Misc**         | `return` (end function)                    |          | Ends function execution, e.g. `if(name === "") return;`     |
| **Misc**         | `return` (return value)                    |          | Returns value, e.g. `return 0`, `return true`, `return xxx` |
| **Misc**         | `typeof {}`                                |          | Returns `"object"`                                          |
| **Misc**         | `typeof null`                              |          | Returns `"object"` (JS quirk)                               |
| **Object**       | `for...in`                                 |          | `for (const key in obj)`                                    |
| **Object**       | `hasOwnProperty()`                         |          | Checks own property only                                    |
